...
//    }
    ...
    {
        String[] s = searchy.split(" ");
        HashMap<Article, Integer> found = new HashMap<>();
        ...
        for(String word: s)
        {
            ...
            try
            {
                ...
                while(rset.next())
                {
                    ...
                    if(relevance > 0)
                    {
                        Article y = new Article(Integer.parseInt(rset.getString(1)), rset.getString(2), rset.getString(3).split(" "),
                                rset.getString(4).split(" "), t, rset.getString(6).split(" "), rset.getString(7));
                        if(found.containsKey(y))
                        {
                            int x = found.get(y);
                            found.put(y, x + relevance);
                        }
                        else
                        {
                            found.put(new Article(Integer.parseInt(rset.getString(1)), rset.getString(2), rset.getString(3).split(" "),
                                    rset.getString(4).split(" "), t, rset.getString(6).split(" "), rset.getString(7)), relevance);
                        }
                    }
                }
        ...
        ArrayList<Pair<Article, Integer>> f = new ArrayList<>();

        Set<Article> keySet = found.keySet();

        for(Article key: keySet)
        {
            f.add(new Pair(key, found.get(key)));
        }
